...
- **Exceptions documentées** : Portfolio → `PortfolioGreeting` (même `.h1`, greeting personnalisé) ;
  ...
  flux document `.product-doc` (`product-doc.css`) sur `/vaults/*`, `/onboarding/*`, `/proof-center`, `/profile`, `/portfolio/[positionId]`, `/legal/*` : H1/H2/H3 + `.stat-value` réduits ; KPI vault en `.h4` ; invest flow 4 steps = shell unique `InvestFlowShell` (cap `64rem` / `product-doc.css`, `min-width:0`, container `invest-flow`, stepper labels masqués `<sm` comme onboarding), stepper dans header, eyebrow Step N/4, footer border-t), wizard actif/done = `bg-(--ct-accent) ct-text-deep`, Step 3 colonne gauche `NestedPanel` ; `/portfolio` (cockpit) et `/admin/*` inchangés.
