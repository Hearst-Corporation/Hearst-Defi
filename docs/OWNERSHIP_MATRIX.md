...
| **Agents / LLM** — `src/lib/llm/*`, `src/lib/agents/*`, `src/app/api/cockpit-chat/route.ts` | Agent-Arch | Proof/Compliance | UI-Layout, DeFi | OpenAI GPT-4.1 (ADR-011), un seul modèle, client `src/lib/llm/openai.ts` (pas de `kimi.ts` / alias `kimi`/`KIMI_*` — retirés). Pas d'outils write/financial. |
