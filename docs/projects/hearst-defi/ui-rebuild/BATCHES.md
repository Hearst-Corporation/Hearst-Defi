...
| 4 | Implémentation — Registre nav complet (D5) | ✅ FAIT (2026-07-04) | 3 entrées `hideFromSubNav: true` ajoutées dans `product-nav-items.ts` : `agent-canvas` (section `dashboard`), `btc-mining-performance-vault` (section `strategy`), `diagnostics` (section `proof-system`) — zéro changement visuel, `typecheck` 0 erreur, `test` 452/453 fichiers (4 échecs pré-existants hors scope, voir `HANDOFF.md`) |
| 5 | Implémentation — Politique data-viz `rgba()` (D6, conditionnel) | ✅ FAIT (2026-07-04) | Les 4 hardcodes `rgba(255,255,255,0.0x)` identifiés batch 1 (`portfolio/page.tsx`, `HcChartCard.tsx`, `time-to-target-chart.tsx`, `recent-activity.tsx` — un 5e site trouvé en cours de route, même pattern) remplacés par 6 primitives `--ct-*` dans `cockpit.css` (`--ct-glass-bevel-subtle`, `--ct-chart-grid-stroke`, `--ct-row-hover-glow{,-faint,-subtle,-strong}`) ; valeurs non unifiées (extraction pure du hardcode, pas de changement visuel). `src/components/ui/chart.tsx` déjà conforme (remap `#ccc`/`#fff` Recharts → tokens). Voir `DECISIONS.md` §Batch 5 |
| 6 | Audit — Discipline breakpoints | ✅ FAIT (2026-07-04) | 3 bugs réels trouvés et corrigés (grilles/flex non responsives dans un conteneur `overflow-hidden`, contenu coupé sous ~1024px/640px) : `dashboard-kpi-strip` (déjà présent dans le tree au démarrage, vérifié et conservé), `monte-carlo-review.tsx` p5/p50/p95, `crew-simulation-section.tsx` Risk/Mode/Gates. Voir `DECISIONS.md` §Batch 6. Tables (`Table` + `overflow-x-auto`) et `Modal` (`w-full max-w-3xl`) audités, déjà corrects |
| 7 | QA visuelle | ✅ FAIT (2026-07-04) | `e2e/visual-qa.spec.ts` (nouveau) — 5 routes (portfolio, vaults, proof-center, admin dashboard, admin strategies) × 3 breakpoints (mobile/tablet/desktop) : heading visible, zéro overflow horizontal, smoke a11y DOM ; + 2 tests d'honnêteté d'état (redirect anonyme → /login). 7/7 verts contre le vrai dev server. Voir `DECISIONS.md` §Batch 7 |
