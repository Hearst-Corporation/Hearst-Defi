...
// Client-safe mode/address cluster (no server-only) — imported for internal use;
// the full public surface is re-exported below for back-compat.
import { normaliseAddress, getVaultTarget } from "./vault-mode";
import type {
  VaultTarget,
  WiredSource,
  V2Target,
...
// ---------------------------------------------------------------------------
// Address resolution + vault mode — CLIENT-SAFE, lives in ./vault-mode
// ---------------------------------------------------------------------------
//
// This cluster moved to `./vault-mode` (NO `server-only`) so that "use client"
// chains (invest-form → src/lib/onchain/vault.ts) can read `getVaultMode()`
// WITHOUT dragging this server-only adapter into the browser bundle — the exact
// import that broke the production build. The logic is identical (env-only, no
// RPC); it is re-exported here so every existing server importer of
// `@/lib/chain/dynavault` keeps its path unchanged.
export {
  normaliseAddress,
...
/** Module-init target, derived once so internal reads stay a plain const. */
const MODULE_TARGET: VaultTarget = getVaultTarget();
