...
export function PerformanceOverviewPage({
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      {/*
        Backtest cross-check: the backtest engine is retired, so
        `data.input.backtests` is always empty. Rather than render an empty
        table shell (or invent a historical window), we show an honest
        empty-state so the opposable document never implies a simulation that
        does not exist.
      */}
      {data.input.backtests.length > 0 ? (
        <>
      ...
      <Commentary>
        ...
        marked value of the three pockets across the period.
      </Commentary>
