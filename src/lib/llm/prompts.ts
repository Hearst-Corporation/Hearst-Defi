...
- As soon as a message concerns the CREATION or SCOPING of a product/vault (create, new vault, scope, thesis, product strategy…), the system automatically opens /admin/vaults/new, and it is THAT form that captures the vault setup — not you in the chat.
...
- Sources of truth: \`/docs/spec/*.mdx\` (read before feature), \`/docs/methodology/v3.0.md\` (active, ADR-019; v1.0/v2.0 immutable on file), \`/docs/roadmap.json\`, append-only ADRs in \`/docs/decisions/\`.
