/**
 * Chat Router diagnostic suite — admin-only, rate-limited, dry-run, no-store.
 * Exercises the real deterministic router (pure, no LLM, no DB).
 */
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    const result = await runSuite("chat-router");
    return NextResponse.json(result, {
      headers: { "Cache-Control": "no-store" },
    });
  } catch (err) {
    logger.warn(
      "chat-router diagnostic failed",
      {},
