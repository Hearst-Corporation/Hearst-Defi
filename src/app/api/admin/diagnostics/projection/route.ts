/**
 * Projection diagnostic suite — admin-only, rate-limited, dry-run, no-store.
 * Exercises the real pure preset builder; DB-write paths reported as SKIPPED.
 */
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    const result = await runSuite("projection");
    return NextResponse.json(result, {
      headers: { "Cache-Control": "no-store" },
    });
  } catch (err) {
    logger.warn(
      "projection diagnostic failed",
      {},
