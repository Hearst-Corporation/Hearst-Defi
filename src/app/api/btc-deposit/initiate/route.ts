...
/**
 * POST /api/btc-deposit/initiate — GATED, VALIDATED, and honestly UNAVAILABLE.
 *
 * ── The gap (spec §5 ↔ contract §2) ─────────────────────────────────────────
 * `docs/VAULT_SPEC_V2.1.md` §5 lists this keeper endpoint ("Initier un dépôt
 ...
 * schema is a STRICT empty object, because inventing an `amount` (or an address,
 ...
 */
const BtcDepositInitiateSchema = z.object({}).strict();
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  const parsed = BtcDepositInitiateSchema.safeParse(body.value);
  if (!parsed.success) {
    return NextResponse.json(
      {
  ...
  logger.info("[keeper] btc-deposit/initiate reached but unsupported", {
