  ...
  return (
    ...
    >
      ...
      <div className="overflow-x-auto overflow-y-auto flex-1 relative z-10">
        {positions.length === 0 ? (
          <p className="body-sm text-[var(--ct-text-muted)] mt-2 italic">No open positions.</p>
        ) : (
          <div className="flex flex-col min-w-150 gap-2">
            <div className="grid grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)] gap-4 pb-2 border-b border-[var(--ct-border-soft)] stat-label">
              <span>Vault</span>
              ...
              {positions.map((p) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    />
                    <span className="body-sm text-[var(--ct-text-primary)] font-medium truncate">
                      {p.vaultName ?? "Unassigned vault"}
                    </span>
                  </div>

                  <span className="tabular-nums body-sm text-right text-[var(--ct-text-body)] mono">
                    {formatUsdCompact(p.principalUsdc)}
                  </span>

                  <span className="tabular-nums body-sm text-[var(--ct-text-strong)] font-semibold text-right mono">
                  ...
                  <div className="text-right body-sm mono">
                    {p.apyLow !== null && p.apyHigh !== null ? (
                      ...
                      <span className="body-xs text-[var(--ct-text-muted)]">Unavailable</span>
                    )}
                  </div>

                  <span className="body-xs tabular-nums text-[var(--ct-text-muted)] text-right mono">
