    ...
    <div className="flex flex-wrap gap-2" role="group" aria-label="Quick actions">
      {presets.map((preset) => (
        <button
          ...
          className={cn(
            "rounded-full border border-[var(--ct-border)] bg-surface-inset px-3 py-2 text-[13px] text-zinc-300 transition-colors duration-150 hover:border-[var(--ct-border-strong)] hover:bg-white/[0.06] hover:text-white",
            !masterAgentEnabled && "cursor-not-allowed opacity-35 hover:border-[var(--ct-border)] hover:bg-surface-inset hover:text-zinc-300",
          )}
        >
          {preset.label}
        </button>
